...
@injectable()
export class SendForgotPasswordPhoneService {
  constructor(
  ...
  async execute({
    ...
    number,
    ddd,
  }: SendForgotPasswordPhoneServiceDTO): Promise<SendForgotPasswordPhoneServiceResponseDTO> {
    ...
    return {
      token: refresh_token,
      countdown: String(config.sms.token.expiration_time),
      user_id: user.id,
    };
  }
}
